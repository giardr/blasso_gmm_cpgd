...
| `run_grid_search.py`      | Script used to generate raw results for Section 4.6 of the article (influence of separation and sample size on performance).<br>Results are later processed in `cpgd_experiments.ipynb`.<br>|
|`results_grid_search_f64.npz`| NPZ file containing the results generated by `run_grid_search.py`.|
|`export_raw_results.py`    | Converts the raw results produced by `run_grid_search.py` (stored as a pickle file) <br>into a compressed NPZ file.<br> |
