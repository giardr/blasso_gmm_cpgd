...
`cpgd_experiments.ipynb`          - Numerical experiments with the CPGD algorithm; results discuted in Section 4 of the article.

`methods.py`                      - Core algorithm implementations

`run_grid_search.py`              – Script used to generate raw results for Section 4.6 of the article (influence of separation and sample size on performance). Results are processed in `cpgd_experiments.ipynb`. The file containing the raw results is available upon request.